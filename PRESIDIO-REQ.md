...
| REQ-8 | **External anchoring** — evidence-store Merkle root periodically committed to a public chain and/or RFC 3161 TSA; tamper-evidence does not require trusting the operator | §3.3 | **Implemented** (`treasury-anchor`: content-addressed receipts, coverage-monotonic log, anchored-prefix verification; chain/TSA *submission adapters* are Phase 1 I/O) · v0.16.0 adds the **anchoring pipeline** per ADR-0002: Merkle aggregation with per-head inclusion proofs (one Bitcoin tx anchors many heads), an evidence-driven submission state machine with confirmation-depth gating and overdue-liveness flagging, emitting receipts into the log; v0.20.0 names the chain-wallet seam (`ChainAnchorSubmitter`: broadcast/poll/calendar-proof) the live wallet implements, with a `treasury-conformance` anchor-submitter contract (monotonic heights, liveness, faithful finalization) it must pass |
...
| ID | Stage | Spec | Phase |
|----|-------|------|-------|
| REQ-20 | Read-only ingestion (on-chain xpubs/view keys + exchange/custodian APIs; full history) | §2, §3.4 | 1 — **chain-source domain layer implemented per ADR-0004** (`treasury-chainsource` v0.18.0: `ChainSource` trait — concrete electrs/Fulcrum/reth/Erigon are deferred I/O shims; chain-agnostic integer-only `AddressHistory` content-hashed; finality-gated **two-source reconciliation** = the §3.3 completeness control, divergence names both sources and blocks close, never auto-reconciles; per-chain `FinalityPolicy` (G-5) — BTC confirmation depth, ETH external finalized; reproducibility gate). **Indexer-output → L1-observation mapping implemented** (v0.19.0 `book::draft_history_observation`: only an *agreed* reconciliation books — the payload commits to the two-source-agreed history hash, settled height and movement count, provenance references the raw-payload evidence hash; a divergence returns `BookError::Diverged` and books nothing; golden-vectored per ADR-0004 action item 5, the residual single point kept minimal and cross-verified). Live node+indexer shims + egress proxy remaining I/O — **the live shims must pass the `treasury-conformance` chain-source contract** (v0.20.0: identity, reproducibility gate, settled-history stability as the tip advances, two-source agreement — the same parameterized assertions the fixtures pass today) |
...
| REQ-25 | GL output — GAAP/IFRS journal entries via the posting protocol (idempotency keys + read-back verification; GL reconciliation is itself L1 evidence) | §2.6, §3.7 | 1 (GAAP); 3 (IFRS) — **entry generation implemented** (`treasury-gaap` v0.11.0). **Posting protocol implemented** (`treasury-posting` v0.12.0). **GL adapter contract + orchestration implemented per ADR-0003** (`treasury-gl` v0.17.0: vendor-agnostic `GlAdapter` trait with **read-back mandatory by type** — a post-only adapter cannot implement it; lifecycle orchestration drives the protocol against any adapter incl. the honest Unknown/read-back recovery; `FixtureGl` exercises the whole loop with fault injection — happy path, lost-ack both directions, dropped/extra entry → VerificationFailed, idempotent retry). Concrete NetSuite → QuickBooks → SAP shims (the I/O over this trait) sequenced per design-partner stack — each must pass the `treasury-conformance` GL-adapter contract (v0.20.0: drive-verifies, two-way read-back fidelity, idempotent submit under the batch key) |
