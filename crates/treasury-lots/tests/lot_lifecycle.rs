...
fn fifo_relieves_oldest_first() {
    let mut book = book();
    let Ok(first) = book.acquire(btc(), exchange(), 100, usd(1_000), usd(10), ts(10), ev(1)) else {
...
fn partial_relief_conserves_basis_exactly() {
    ...
    let Ok(first) = book.dispose(&btc(), &exchange(), 1, ReliefMethod::Fifo, ev(9), usd(0)) else {
        unreachable!("dispose must succeed");
    };
    // floor(10 * 1 / 3) = 3; residual lot keeps 7 over 2 atoms.
    assert_eq!(first.total_basis_relieved, usd(3));

    let Ok(second) = book.dispose(&btc(), &exchange(), 2, ReliefMethod::Fifo, ev(9), usd(0)) else {
        unreachable!("dispose must succeed");
    };
...
fn fees_stay_decomposed_from_basis() {
    ...
    assert_eq!(
        result.reliefs.first().map(|r| r.fee_relieved.clone()),
        Some(usd(25))
    );
}
...
fn specific_identification_relieves_named_lots() {
    let mut book = book();
    let Ok(_first) = book.acquire(btc(), exchange(), 100, usd(1_000), usd(0), ts(10), ev(1)) else {
        unreachable!("acquire must succeed");
    };
    let Ok(second) = book.acquire(btc(), exchange(), 100, usd(4_000), usd(0), ts(20), ev(2)) else {
        unreachable!("acquire must succeed");
    };
...
fn transfer_preserves_basis_and_acquisition_date() {
    let mut book = book();
    let Ok(origin) = book.acquire(btc(), cold(), 100, usd(1_000), usd(10), ts(10), ev(1)) else {
        unreachable!("acquire must succeed");
    };
...
fn golden_hashes_match_independent_implementation() {
    ...
    let acquired = book.acquire(btc(), VenueId::new("v"), 7, usd(70), usd(1), ts(10), ev(1));
    assert_eq!(
...
fn lots_hash_is_deterministic_and_state_sensitive() {
    ...
    assert_eq!(
        book.lots_hash(),
        after_acquire,
        "hash is a pure function of state"
    );
}
