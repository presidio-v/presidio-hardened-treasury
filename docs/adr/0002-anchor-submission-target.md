...
6. [x] Capture the confirmation-depth used for "anchored" as part of, or alongside, the per-chain finality policy (G-5, §3.5) so the anchoring threshold is itself a documented artifact. *(done — `treasury-anchor::AnchorPolicy` is a content-addressed sibling of `FinalityPolicy`; `AnchorPipeline::finalize` takes the policy and stamps its hash into every receipt, so a change to the threshold changes the receipt hash and is in the audit trail. Located in `treasury-anchor` rather than on the per-chain `FinalityPolicy` because the anchoring depth is a single Bitcoin-anchoring parameter — meaningless on an Ethereum finality policy — and the anchoring layer must not depend on the chain-ingestion layer.)*
